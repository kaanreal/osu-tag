...
namespace OsuTag.Services
{
    internal class OsuMapScanner
    {
        ...
        public IEnumerable<OsuMap> ScanSingleFolder(string folder)
        {
            ...
            try
            {
                ...
                foreach (var osuFile in osuFiles)
                {
                    ...
                    // Yield briefly every FileYieldBatch files to avoid creating large IO bursts
                    fileCounter++;
        ...
        /// <summary>
        /// Async wrapper for scanning a single folder.
        /// </summary>
        ...
        private OsuMap? CreateMapFromDifficulties(string folder, List<(string osuFile, Dictionary<string, string?> metadata)> diffs, string audioKey)
        {
            ...
            foreach (var (osuFile, diffMetadata) in diffs)
            {
                ...
                if (diffMetadata.TryGetValue("Version", out var version) && !string.IsNullOrEmpty(version))
                {
                    diffName = version;
                }

                // Extract specific metadata for this difficulty
                string? diffTitle = diffMetadata.GetValueOrDefault("Title");
                string? diffArtist = diffMetadata.GetValueOrDefault("Artist");
                
                string? diffCoverPath = null;
                if (diffMetadata.TryGetValue("BackgroundFile", out var diffBgFile) && !string.IsNullOrEmpty(diffBgFile))
                {
                    string bgPath = Path.Combine(folder, diffBgFile);
                    if (File.Exists(bgPath))
                ...
                if (diffMetadata.TryGetValue("PreviewTime", out var diffPreviewStr) &&
                    int.TryParse(diffPreviewStr, out int diffParsedTime))
                {
                    diffPreviewTime = diffParsedTime;
                }
                else
                {
                    diffPreviewTime = previewTime;
                }
                ...
                difficulties.Add(new OsuMapDifficulty
                {
                    ...
                    Title = diffTitle,
                    Artist = diffArtist,
                    CoverPath = diffCoverPath,
                    PreviewTime = diffPreviewTime
                });
            }
        ...
        private static Dictionary<string, string?>? ParseOsuFile(string filePath)
        {
            try
            {
                ...
                // Open the file with FileOptions.SequentialScan for better performance
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (inEventsSection && !foundBackground && trimmedLine.StartsWith("0,0,"))
                    {
                        ...
                        if (parts.Length >= 3)
                        {
                            var bgFileValue = parts[2].Trim('"');
                            if (!string.IsNullOrEmpty(bgFileValue) &&
                                (bgFileValue.EndsWith(".jpg", StringComparison.OrdinalIgnoreCase) ||
                                 bgFileValue.EndsWith(".jpeg", StringComparison.OrdinalIgnoreCase) ||
                                 bgFileValue.EndsWith(".png", StringComparison.OrdinalIgnoreCase)))
                            {
                                metadata["BackgroundFile"] = bgFileValue;

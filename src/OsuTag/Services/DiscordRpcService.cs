...
namespace OsuTag.Services
{
    internal static class DiscordRpcService
    {
        ...
        public static void Initialize()
        {
            ...
            if (!SettingsService.Settings.DiscordRpcEnabled) return;

            _client = new DiscordRpcClient("1456772447578886387"); // Discord application's client ID
            _client.Initialize();
            UpdateStatus("idle", 0);
        }
        ...
        {
            // Check if Discord RPC is enabled
            if (!SettingsService.Settings.DiscordRpcEnabled)
            {
